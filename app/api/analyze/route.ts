    ...
    try {
      ...
      const analysisResult: AnalysisResult = {
        ...
        summary: {
          ...
          positiveRatio: normalizedData.filter(row => row.sentiment.includes('正面')).length / normalizedData.length,
          neutralRatio: normalizedData.filter(row => row.sentiment.includes('中性')).length / normalizedData.length,
          negativeRatio: normalizedData.filter(row => row.sentiment.includes('負面')).length / normalizedData.length,
          averageRating: normalizedData.reduce((acc, row) => acc + row.rating, 0) / normalizedData.length
        }
      };
